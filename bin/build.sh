...
# Compile Acorn Electron AP6 target (config-less standalone; *CONFIGURE lives in C.I2CEAP6)
echo ""
...
./bin/beebasm -i ./src/I2CBeeb.asm -do ./src/out/out.ssd -title I2C \
    ...
    -D INC_CONFIG=0 \
    -o "I2CEAP6" \
    -d -labels ./src/out/I2CEAP6.labels
# Extract from ssd to output folder
...
############################################################
# Build AP6 configure ROM (*CONFIGURE/*STATUS via PCF8583 NVRAM)
############################################################

# Compile configure-enabled Electron AP6 target
...
# Copy production ROMs (with BeebAsm debug labels for ROM unit tests) to dist folder
cp ./src/out/b/I2CB ./dist/i2cb.rom
cp ./src/out/I2CB.labels ./dist/i2cb.labels
cp ./src/out/e/I2CE ./dist/i2ce.rom
cp ./src/out/I2CE.labels ./dist/i2ce.labels
cp ./src/out/ap6/I2CEAP6 ./dist/i2ceap6.rom
cp ./src/out/I2CEAP6.labels ./dist/i2ceap6.labels
...
# Copy AP6 configure ROM to dist folder
...
# Copy AP6 configure ROM to dev/eap6 for hardware testing
echo ""
echo "*** Copying AP6 Configure ROM to Dev Folder ***"
