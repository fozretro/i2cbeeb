...
The Time & Config integration code is dynamically pulled from the [Time & Config. ROM repository](https://codeberg.org/Barneyntd/Time-Config.) during the build process and is not duplicated in this repository. The build scripts (`/bin/time-config/extract.sh`) clone the Time & Config repository, extract the necessary source files, and apply selective modifications (such as removing unsupported features) before inclusion in the ROM builds. For the full Time & Config ROM with all features, readers should refer to the [Time & Config. repository](https://codeberg.org/Barneyntd/Time-Config.) directly. 
